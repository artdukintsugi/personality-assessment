...
function facetDomain(f) {
  for (const [d, fs] of Object.entries(DF)) { if (fs.includes(f)) return d; }
  return null;
}
...
function scoreFacets(answers) {
  ...
  Object.entries(FM).forEach(([f, items]) => {
    const vals = items.map(i => answers[i]).filter(v => v !== undefined && v !== null);
    r[f] = vals.length ? vals.reduce((a,b) => a+b, 0) / vals.length : 0;
  });
  return r;
}

function scoreDomains(facetScores) {
  const r = {};
  Object.entries(DF).forEach(([d, fs]) => {
    const vals = fs.map(f => facetScores[f] || 0);
    r[d] = vals.length ? vals.reduce((a,b) => a+b, 0) / vals.length : 0;
  });
  return r;
}
...
// ═══════════════════════════════════════════════════════════
// DIAGNOSTIC PROFILES — all DSM-5 Section III + related
// ═══════════════════════════════════════════════════════════
const DIAG_PROFILES = [
  {
    id: "bpd", name: "Hraniční porucha osobnosti (BPD)", color: "#F87171",
    desc: "DSM-5 alternativní model: Emoční labilita, Impulzivita, Separační nejistota, Depresivita, Hostilita, Riskování, Úzkostnost.",
    facets: ["Emoční labilita", "Impulzivita", "Separační nejistota", "Depresivita", "Hostilita", "Riskování", "Úzkostnost"],
    weights: { "Emoční labilita": 1.5, "Impulzivita": 1.3, "Separační nejistota": 1.3, "Depresivita": 1.0, "Hostilita": 1.2, "Riskování": 0.8, "Úzkostnost": 0.9 },
    threshold: 1.3,
  },
  {
    id: "npd", name: "Narcistická porucha osobnosti (NPD)", color: "#FBBF24",
    desc: "DSM-5 alternativní model: Grandiozita, Vyhášení pozornosti. Antagonismus celkově.",
    facets: ["Grandiozita", "Vyhášení pozornosti", "Manipulativnost", "Bezcitnost", "Klamavost"],
    weights: { "Grandiozita": 1.5, "Vyhášení pozornosti": 1.3, "Manipulativnost": 1.0, "Bezcitnost": 0.8, "Klamavost": 0.7 },
    threshold: 1.3,
  },
  {
    id: "aspd", name: "Antisociální porucha osobnosti (ASPD)", color: "#EF4444",
    desc: "DSM-5 alternativní model: Manipulativnost, Klamavost, Bezcitnost, Hostilita, Impulzivita, Riskování, Nezodpovědnost.",
    facets: ["Manipulativnost", "Klamavost", "Bezcitnost", "Hostilita", "Impulzivita", "Riskování", "Nezodpovědnost"],
    weights: { "Manipulativnost": 1.3, "Klamavost": 1.3, "Bezcitnost": 1.5, "Hostilita": 1.2, "Impulzivita": 1.0, "Riskování": 1.0, "Nezodpovědnost": 1.2 },
    threshold: 1.3,
  },
  {
    id: "avpd", name: "Vyhýbavá porucha osobnosti (AvPD)", color: "#60A5FA",
    desc: "DSM-5 alternativní model: Úzkostnost, Stažení, Anhedonie, Vyhýbání se intimitě.",
    facets: ["Úzkostnost", "Stažení", "Anhedonie", "Vyhýbání se intimitě", "Separační nejistota", "Submisivita"],
    weights: { "Úzkostnost": 1.5, "Stažení": 1.3, "Anhedonie": 1.0, "Vyhýbání se intimitě": 1.3, "Separační nejistota": 0.8, "Submisivita": 0.9 },
    threshold: 1.3,
  },
  {
    id: "ocpd", name: "Obsedantně-kompulzivní porucha osobnosti (OCPD)", color: "#A78BFA",
    desc: "DSM-5 alternativní model: Rigidita, Perseverace + Restriktivní afektivita, Vyhýbání se intimitě (neg.).",
    facets: ["Rigidita", "Perseverace", "Restriktivní afektivita", "Úzkostnost", "Submisivita"],
    weights: { "Rigidita": 1.5, "Perseverace": 1.3, "Restriktivní afektivita": 1.0, "Úzkostnost": 0.8, "Submisivita": 0.7 },
    threshold: 1.3,
  },
  {
    id: "stpd", name: "Schizotypní porucha osobnosti (StPD)", color: "#C084FC",
    desc: "DSM-5 alternativní model: Excentricita, Percepční dysregulace, Neobvyklé přesvědčení, Stažení, Podezřívavost.",
    facets: ["Excentricita", "Percepční dysregulace", "Neobvyklé přesvědčení", "Stažení", "Podezřívavost", "Restriktivní afektivita"],
    weights: { "Excentricita": 1.5, "Percepční dysregulace": 1.3, "Neobvyklé přesvědčení": 1.3, "Stažení": 1.0, "Podezřívavost": 1.0, "Restriktivní afektivita": 0.7 },
    threshold: 1.3,
  },
  {
    id: "szpd", name: "Schizoidní rysy osobnosti", color: "#94A3B8",
    desc: "Stažení, Anhedonie, Restriktivní afektivita, Vyhýbání se intimitě — omezená emoční angažovanost a sociální izolace.",
    facets: ["Stažení", "Anhedonie", "Restriktivní afektivita", "Vyhýbání se intimitě", "Depresivita"],
    weights: { "Stažení": 1.5, "Anhedonie": 1.3, "Restriktivní afektivita": 1.3, "Vyhýbání se intimitě": 1.2, "Depresivita": 0.8 },
    threshold: 1.3,
  },
  {
    id: "ppd", name: "Paranoidní rysy osobnosti", color: "#FB923C",
    desc: "Podezřívavost, Hostilita, Stažení, Vyhýbání se intimitě — mistrust a interpretace zlomyslnosti.",
    facets: ["Podezřívavost", "Hostilita", "Stažení", "Vyhýbání se intimitě", "Restriktivní afektivita"],
    weights: { "Podezřívavost": 1.5, "Hostilita": 1.3, "Stažení": 1.0, "Vyhýbání se intimitě": 1.0, "Restriktivní afektivita": 0.7 },
    threshold: 1.3,
  },
  {
    id: "hpd", name: "Histrionské rysy osobnosti", color: "#F472B6",
    desc: "Vyhášení pozornosti, Emoční labilita, Manipulativnost, Separační nejistota — dramatičnost a potřeba pozornosti.",
    facets: ["Vyhášení pozornosti", "Emoční labilita", "Manipulativnost", "Separační nejistota", "Submisivita"],
    weights: { "Vyhášení pozornosti": 1.5, "Emoční labilita": 1.2, "Manipulativnost": 1.0, "Separační nejistota": 1.0, "Submisivita": 0.8 },
    threshold: 1.3,
  },
  {
    id: "dpd", name: "Závislé rysy osobnosti (DPD)", color: "#38BDF8",
    desc: "Submisivita, Separační nejistota, Úzkostnost — nadměrná potřeba péče a strach z opuštění.",
    facets: ["Submisivita", "Separační nejistota", "Úzkostnost", "Depresivita"],
    weights: { "Submisivita": 1.5, "Separační nejistota": 1.5, "Úzkostnost": 1.0, "Depresivita": 0.8 },
    threshold: 1.3,
  },
  {
    id: "depressive", name: "Depresivní rysy osobnosti", color: "#64748B",
    desc: "Depresivita, Anhedonie, Úzkostnost, Submisivita — trvalá pesimistická nálada a sebekritičnost.",
    facets: ["Depresivita", "Anhedonie", "Úzkostnost", "Submisivita", "Restriktivní afektivita"],
    weights: { "Depresivita": 1.5, "Anhedonie": 1.3, "Úzkostnost": 1.0, "Submisivita": 0.8, "Restriktivní afektivita": 0.7 },
    threshold: 1.3,
  },
  {
    id: "adhd", name: "ADHD — indikátory (ne-PD)", color: "#34D399",
    desc: "PID-5 primárně neměří ADHD, ale Disinhibice (Roztříštěnost, Impulzivita, Nezodpovědnost) koreluje s ADHD fenomény.",
    facets: ["Roztříštěnost", "Impulzivita", "Nezodpovědnost", "Riskování"],
    weights: { "Roztříštěnost": 1.5, "Impulzivita": 1.3, "Nezodpovědnost": 1.2, "Riskování": 0.8 },
    threshold: 1.3,
  },
  {
    id: "did", name: "Disociace / DID — indikátory (ne-PD)", color: "#E879F9",
    desc: "PID-5 přímo neměří disociaci, ale Percepční dysregulace a Neobvyklé přesvědčení zachycují disociativní fenomény.",
    facets: ["Percepční dysregulace", "Neobvyklé přesvědčení", "Excentricita", "Depresivita"],
...
function scoreDiagnostics(fScores) {
  return DIAG_PROFILES.map(prof => {
    ...
    prof.facets.forEach(f => {
      const w = prof.weights[f] || 1;
      totalW += w;
      totalS += (fScores[f] || 0) * w;
    });
...
export default function App() {
  ...
  // Auto-save to history when entering results
  useEffect(() => {
    if (mode === 'pid5_results' && Object.keys(answers).length === 220) {
      ...
      if (!recent || recent.type !== 'pid5' || Date.now() - recent.id > 60000) {
        ...
        saveToHistory('pid5', {
          ...
          fullData: { domeny: scoreDomains(scoreFacets(answers)), facety: scoreFacets(answers), diagnostika: diags.map(d => ({id:d.id,name:d.name,score:d.score,flag:d.flag})), odpovedi: answers }
        });
      }
    }
    if (mode === 'lpfs_results' && Object.keys(lpfsAns).length === 80) {
      ...
      if (!recent || recent.type !== 'lpfs' || Date.now() - recent.id > 60000) {
        ...
        saveToHistory('lpfs', { score: avg, fullData: { prumer: avg, odpovedi: lpfsAns } });
      }
    }
  ...
  // Debug: fill with sample random answers
  const fillSample = useCallback(() => {
    const s = {};
    for (let i = 0; i < 220; i++) {
      const r = Math.random();
      s[i] = r < 0.2 ? 0 : r < 0.5 ? 1 : r < 0.8 ? 2 : 3;
    }
    setAnswers(s);
    setIdx(219);
  ...
  const fillSampleLpfs = useCallback(() => {
    const s = {};
    for (let i = 0; i < 80; i++) {
      const r = Math.random();
      s[i] = r < 0.2 ? 1 : r < 0.5 ? 2 : r < 0.8 ? 3 : 4;
    }
    setLpfsAns(s);
    setLpfsIdx(79);
  ...
  const DiagCard = ({ diag, fScores }) => {
    const { name, color, desc, facets: dFacets, score, flag } = diag;
    return (
      <div className={`mb-4 p-4 rounded-xl border transition-all ${flag ? 'border-opacity-40' : 'border-gray-700/30 bg-gray-800/20'}`}
        style={flag ? { borderColor: color + '50', background: color + '08' } : {}}>
        <div className="flex items-center gap-2 mb-2">
          <div className="w-3 h-3 rounded-full" style={{ background: color }} />
          <span className="font-semibold text-sm" style={{ color: flag ? color : '#9CA3AF' }}>{name}</span>
          <span className="text-xs px-2 py-0.5 rounded-full ml-auto"
            style={{ background: flag ? color + '20' : '#374151', color: flag ? color : '#6B7280' }}>
          ...
          {dFacets.map(f => {
            ...
            return (
              <div key={f} className="flex items-center gap-2">
                <div className="w-36 text-xs text-gray-400 truncate">↳ {f}</div>
  ...
  if (mode === "menu") return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col items-center justify-center p-6 font-sans">
      ...
      <div className="flex flex-col gap-4 w-full max-w-sm">
        ...
        {/* Debug / Sample buttons */}
        <div className="border-t border-gray-800 pt-4 mt-4">
          <div className="text-xs text-gray-600 mb-2">🔧 Debug — vzorek dat</div>
          <div className="flex gap-2">
            <button onClick={fillSample} className="flex-1 p-3 rounded-xl bg-gray-800/50 border border-gray-700/30 text-gray-400 text-xs hover:text-gray-200 hover:border-gray-600 transition-all">
              🎲 PID-5 random
            </button>
            <button onClick={fillSampleLpfs} className="flex-1 p-3 rounded-xl bg-gray-800/50 border border-gray-700/30 text-gray-400 text-xs hover:text-gray-200 hover:border-gray-600 transition-all">
              🎲 LPFS random
            </button>
            <button onClick={() => { setAnswers({}); setIdx(0); setLpfsAns({}); setLpfsIdx(0); }} className="flex-1 p-3 rounded-xl bg-gray-800/50 border border-red-900/30 text-gray-500 text-xs hover:text-red-300 hover:border-red-700 transition-all">
              🗑 Reset vše
            </button>
          </div>
        </div>
                ...
                {history.map((h) => (
                  <div key={h.id} className="p-3 rounded-xl bg-gray-800/40 border border-gray-700/30 text-xs">
                    <div className="flex items-center justify-between mb-1">
                      <span className={h.type === 'pid5' ? 'text-purple-400 font-semibold' : 'text-blue-400 font-semibold'}>
                        {h.type === 'pid5' ? 'PID-5' : 'LPFS-SR'}
                      </span>
                      <span className="text-gray-600">{new Date(h.date).toLocaleString('cs-CZ')}</span>
                    </div>
                    {h.type === 'pid5' && h.topDiags && (
                    ...
                    <div className="flex gap-2 mt-2">
                      <button onClick={() => {
                        ...
                        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${h.type}_${h.date.slice(0,10)}.json`; a.click();
                      }} className="text-gray-500 hover:text-gray-300">Export JSON</button>
                      <button onClick={() => {
                        if (confirm('Smazat tento z\u00E1znam?')) setHistory(prev => prev.filter(x => x.id !== h.id));
                      }} className="text-gray-600 hover:text-red-400">Smazat</button>
                    </div>
                  </div>
                ))}
                <button onClick={() => { if (confirm('Smazat celou historii?')) setHistory([]); }}
                  className="w-full p-2 rounded-xl text-xs text-gray-600 hover:text-red-400 border border-gray-800 hover:border-red-900/30 transition-all">
                  🗑 Vymazat historii
                </button>
              </div>
            )}
  ...
  if (mode === "pid5_results") return (
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8 font-sans">
      <div className="max-w-4xl mx-auto">
        ...
        {/* ═══ ALL DIAGNOSTICS ═══ */}
        <div className="bg-gray-900/60 rounded-2xl border border-gray-800 p-6 mb-8 backdrop-blur-xl">
          <h3 className="text-lg font-semibold text-gray-300 mb-2">Diagnostické profily — všechny</h3>
          <p className="text-xs text-gray-500 mb-6">Orientační mapování na všechny poruchy osobnosti dle DSM-5 alternativního modelu + komorbidní indikátory (ADHD, DID). Nenahrazuje klinické hodnocení.</p>

          {/* Summary bar */}
          <div className="mb-6 p-4 rounded-xl bg-gray-800/40 border border-gray-700/30">
            <div className="text-xs text-gray-500 mb-2">Seřazeno dle skóre (nejvyšší první):</div>
            <div className="space-y-1">
              {diagnostics.map(d => (
                <div key={d.id} className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full shrink-0" style={{ background: d.flag ? d.color : '#374151' }} />
                  <div className="w-44 text-xs truncate" style={{ color: d.flag ? d.color : '#6B7280' }}>
                    {d.name.split('(')[0].trim()}
                    {d.name.includes('(') && <span className="text-gray-600"> ({d.name.split('(')[1]}</span>}
                  </div>
                  <div className="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{ width: `${(d.score/3)*100}%`, background: d.color, opacity: d.flag ? 1 : 0.4 }} />
                  </div>
                  <div className="w-10 text-right text-xs font-mono" style={{ color: d.flag ? d.color : '#6B7280' }}>{d.score.toFixed(2)}</div>
                  <div className="w-16 text-right text-xs" style={{ color: d.flag ? d.color : '#4B5563' }}>
                    {d.flag ? '⚠ Zvýšené' : d.score >= 1.0 ? 'Mírné' : 'Nízké'}
                  </div>
                </div>
              ))}
            </div>
          ...
          {diagnostics.filter(d => d.flag).length > 0 && (
            <div className="mb-4">
              <div className="text-sm font-semibold text-gray-400 mb-3">⚠ Zvýšené profily:</div>
              {diagnostics.filter(d => d.flag).map(d => <DiagCard key={d.id} diag={d} fScores={facetScores} />)}
            </div>
          )}
          {diagnostics.filter(d => !d.flag && d.score >= 0.8).length > 0 && (
            <div className="mb-4">
              <div className="text-sm font-semibold text-gray-500 mb-3">Subklinické / v normě:</div>
              {diagnostics.filter(d => !d.flag && d.score >= 0.8).map(d => <DiagCard key={d.id} diag={d} fScores={facetScores} />)}
            </div>
          )}
          {diagnostics.filter(d => d.score < 0.8).length > 0 && (
            <div>
              <div className="text-sm font-semibold text-gray-600 mb-3">Nízké:</div>
              {diagnostics.filter(d => d.score < 0.8).map(d => <DiagCard key={d.id} diag={d} fScores={facetScores} />)}
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-900/60 rounded-2xl border border-gray-800 p-6 mb-8 backdrop-blur-xl">
          <h3 className="text-lg font-semibold text-gray-300 mb-4">Celkové zhodnocení</h3>
          <p className="text-sm text-gray-400 leading-relaxed">
            PID-5 je dimenzionální nástroj — měří maladaptivní osobnostní rysy na kontinuu, nikoli kategorické diagnózy.
            Výsledky slouží jako podklad pro klinický rozhovor. Pro ADHD diagnostiku jsou klíčové kognitivní testy (CPT, d2, WAIS-IV).
            Pro DID je nutné specifické vyšetření (SCID-D, DES-II).
          </p>
          <p className="text-sm text-gray-400 leading-relaxed mt-2">
            <strong>DSM-5 alternativní model</strong> přímo definuje kritéria pro 6 poruch osobnosti: BPD, NPD, ASPD, AvPD, OCPD, StPD.
            Ostatní profily (schizoidní, paranoidní, histrionské, závislé, depresivní) jsou mapovány na základě facetových konstelací z literatury.
          </p>
        </div>

        {/* Zdroje */}
        <div className="bg-gray-900/60 rounded-2xl border border-gray-800 p-6 mb-8 backdrop-blur-xl">
          <h3 className="text-lg font-semibold text-gray-300 mb-4">Zdroje a literatura</h3>
          <div className="space-y-3 text-sm text-gray-400">
            <div>
              <p className="text-gray-300 font-medium">Krueger et al. (2012)</p>
              <p className="text-xs">Initial construction of a maladaptive personality trait model and inventory for DSM-5. <em>Psychological Medicine, 42</em>(9).</p>
            </div>
            <div>
              <p className="text-gray-300 font-medium">APA (2013) — DSM-5 Section III</p>
              <p className="text-xs">Alternative Model for Personality Disorders — dimenzionální model, základ PID-5.</p>
            </div>
            <div>
              <p className="text-gray-300 font-medium">Bach, Sellbom & Simonsen (2018)</p>
              <p className="text-xs">PID-5 in clinical vs. nonclinical — cutoff skóry pro klinickou významnost.</p>
            </div>
            <div>
              <p className="text-gray-300 font-medium">Morey (2017) — LPFS-SR</p>
              <p className="text-xs">Self-report Level of Personality Functioning Scale.</p>
            </div>
            <div>
              <p className="text-gray-300 font-medium">Widiger et al. (2019)</p>
              <p className="text-xs">PID-5 profiles for DSM-5 personality disorders — facetové mapování pro všech 10 PD.</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-3 mb-12">
          <button onClick={() => {
            const data = { domeny: domainScores, facety: facetScores, diagnostika: diagnostics.map(d => ({id: d.id, name: d.name, score: d.score, flag: d.flag})), odpovedi: answers };
            const blob = new Blob([JSON.stringify(data, null, 2)], {type:'application/json'});
            const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'pid5_vysledky.json'; a.click();
          }} className="px-6 py-3 bg-purple-600 hover:bg-purple-500 rounded-xl text-white font-semibold transition-all">
            Exportovat JSON
          </button>
          <button onClick={() => setMode("menu")} className="px-6 py-3 bg-gray-800 hover:bg-gray-700 rounded-xl text-gray-300 font-semibold transition-all">
            ← Menu
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8 font-sans">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex gap-3">
          <button onClick={() => {
            const blob = new Blob([JSON.stringify({prumer: lpfsTotal, odpovedi: lpfsAns}, null, 2)], {type:'application/json'});
            const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'lpfs_vysledky.json'; a.click();
        ...
        <div className="w-full max-w-lg">
          ...
          {isPid && domain && (
            <div className="flex gap-2 mb-3 flex-wrap">
              {facets.map(f => (
                <span key={f} className="text-xs px-2.5 py-1 rounded-full border" style={{borderColor: DC[domain] + '60', color: DC[domain], background: DC[domain] + '15'}}>
                  {f}
                </span>
              ))}
            </div>
          ...
          <div className={`rounded-2xl border p-6 md:p-8 backdrop-blur-xl ${isPid ? 'bg-purple-950/30 border-purple-500/20' : 'bg-blue-950/30 border-blue-500/20'}`}>
            ...
            <p className="text-lg md:text-xl font-medium leading-relaxed mb-8 text-gray-100">{curQ[curI]}</p>
              ...
              {optLabels.map((label, i) => {
                ...
                return (
                  <button key={i} onClick={() => answer(val)}
                    ...
                    className={`p-4 rounded-xl text-left transition-all border font-medium text-sm
                      ${isSelected
                        ? (isPid ? 'bg-purple-600/40 border-purple-400/60 text-purple-200' : 'bg-blue-600/40 border-blue-400/60 text-blue-200')
                        : 'bg-gray-900/40 border-gray-700/40 text-gray-300 hover:border-gray-500/60 hover:bg-gray-800/40'
                      }`}
                  >
                  ...
                  return (
                    <div key={d} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 bg-gray-800 rounded-full h-2 overflow-hidden relative">
                        ...
                        {hoveredVal !== null && (
                          <div className="absolute inset-0 h-full rounded-full transition-all duration-200" style={{width: `${(v/3)*100}%`, background: DC[d]}} />
                        )}
                      </div>
                      <div className="w-10 text-right text-xs font-mono text-gray-300">{v.toFixed(2)}</div>
                    ...
                    return (
                      <div key={f} className="flex items-center gap-2">
                        ...
                        <div className="flex-1 bg-gray-800 rounded-full h-1.5 overflow-hidden relative">
                          ...
                          {hoveredVal !== null && (
                            <div className="absolute inset-0 h-full rounded-full transition-all duration-200" style={{width: `${(v/3)*100}%`, background: SEV_CLR(v)}} />
                          )}
                        </div>
                        <div className="w-10 text-right text-xs font-mono text-gray-400">{v.toFixed(2)}</div>
                        <div className={`w-12 text-right text-xs font-mono ${diff > 0 ? 'text-red-400' : diff < 0 ? 'text-green-400' : 'text-gray-700'}`}>
            ...
            <div className="mt-4 rounded-2xl border border-gray-700/30 bg-gray-900/40 backdrop-blur-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs text-gray-500 flex items-center gap-2">
                  ...
                  Diagnostické profily — live
                </div>
                <button onClick={() => setShowDiagLive(!showDiagLive)} className="text-xs text-gray-600 hover:text-gray-400">
                  {showDiagLive ? '\u25BE Sbalit' : '\u25B8 Rozbalit'}
                </button>
              </div>
              {showDiagLive && (
                <div className="space-y-1">
                  {liveDiags.map(d => {
                    ...
                    return (
                      <div key={d.id} className="flex items-center gap-2 py-0.5">
                        <div className="w-3 h-3 rounded-full shrink-0" style={{ background: d.flag ? d.color : '#374151' }} />
                        <div className="w-32 text-xs truncate" style={{ color: d.flag ? d.color : '#6B7280' }}>
                          {d.name.split('(')[0].split('—')[0].trim()}
                        </div>
                        <div className="flex-1 bg-gray-800 rounded-full h-1.5 overflow-hidden relative">
                          <div className="absolute inset-0 h-full rounded-full transition-all duration-300"
                            style={{ width: `${((baseDiag?.score || 0)/3)*100}%`, background: d.color, opacity: hoveredVal !== null ? 0.4 : 1 }} />
                          {hoveredVal !== null && (
                            <div className="absolute inset-0 h-full rounded-full transition-all duration-200"
                              style={{ width: `${(d.score/3)*100}%`, background: d.color }} />
                          )}
                        </div>
                        <div className="w-10 text-right text-xs font-mono" style={{ color: d.flag ? d.color : '#6B7280' }}>{d.score.toFixed(2)}</div>
                        <div className={`w-10 text-right text-xs font-mono ${diff > 0 ? 'text-red-400' : diff < 0 ? 'text-green-400' : 'text-gray-700'}`}>
                          {diff !== 0 ? (diff > 0 ? '+' : '') + diff.toFixed(2) : ''}
                        </div>
                      </div>
                    );
          ...
          <div className="flex justify-between mt-4">
            <button onClick={() => setCurI(Math.max(0, curI - 1))} disabled={curI === 0}
              className="text-sm text-gray-500 hover:text-gray-300 disabled:opacity-30">← Předchozí</button>
            <div className="flex gap-1">
              {Array.from({length: Math.min(10, Math.ceil((curI+1)/10)*10) - Math.floor(curI/10)*10}, (_, j) => {
                ...
                return <button key={n} onClick={() => setCurI(n)}
                  className={`w-6 h-6 rounded-full text-xs flex items-center justify-center transition-all
                    ${n === curI ? (isPid ? 'bg-purple-500 text-white' : 'bg-blue-500 text-white') : has ? 'bg-gray-700 text-gray-300' : 'bg-gray-800/50 text-gray-600'}`}
                >{n+1}</button>;
              })}
            </div>
            <button onClick={() => setCurI(Math.min(curQ.length - 1, curI + 1))} disabled={curI >= curQ.length - 1}
              className="text-sm text-gray-500 hover:text-gray-300 disabled:opacity-30">Další →</button>
          </div>

          {/* Quick finish */}
          {answered >= (isPid ? Q.length : LPFS_Q.length) && (
            <button onClick={() => setMode(isPid ? 'pid5_results' : 'lpfs_results')}
              className={`w-full mt-6 p-4 rounded-xl font-semibold text-lg transition-all ${isPid ? 'bg-purple-600 hover:bg-purple-500' : 'bg-blue-600 hover:bg-blue-500'}`}>
              Zobrazit výsledky
            </button>
          )}
        </div>
