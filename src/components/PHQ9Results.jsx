            ...
            <div className="frosted rounded-2xl p-6 w-full max-w-md shadow-xl">
              ...
              {compareType === 'other' && (
                <div className="mb-4">
                  ...
                  <input type="text" className="w-full p-2 rounded bg-white/[0.04] border border-white/[0.08] text-xs text-gray-200 mb-2" placeholder="12" value={otherLabel} onChange={e => setOtherLabel(e.target.value)} />
                  <button onClick={() => {
                    const val = parseInt(otherLabel);
                    if (!isNaN(val)) setOtherScore(val);
                    else alert('Invalid number');
                  }} className="px-3 py-1 rounded bg-emerald-600 text-white text-xs font-semibold">{lang === 'cs' ? 'Načíst' : 'Load'}</button>
                </div>
              )}
              {otherScore !== null && (
                <div className="mb-4">
                  ...
                  <div className="relative h-3 bg-white/[0.06] rounded-full mt-2">
        ...
        {showLive && (
          ...
          <div className="space-y-3">
            {questions.map((q, i) => {
              ...
              return (
                <div key={i} className={`p-3 rounded-xl border ${isItem9 && val > 0 ? 'border-red-500/30 bg-red-950/10' : 'border-white/[0.05] bg-white/[0.02]'}`}>
